...
from .config import CHICAGO_NEARSHORE, MPH_TO_KNOTS, NDBC_STATION, WEEKDAYS
...
def chicago_forecast(label: str, include_details: bool = False) -> Dict:
    """
    ...
    sun_times = calculate_sun_times(
        CITIES["chicago"]["lat"],
        CITIES["chicago"]["lon"],
        _label_to_date(label),
    )

    rating = compute_rating(wrng, waves, sky)
    rating_breakdown = compute_rating_breakdown(wrng, waves, sky) if include_details else None
...
def marine_city_forecast(city_key: str, label: str, include_details: bool = False) -> Dict:
    """
    ...
    # Get sun times
    sun_times = calculate_sun_times(meta["lat"], meta["lon"], _label_to_date(label))
...
def grid_city_forecast(city_key: str, label: str, include_details: bool = False) -> Dict:
    """
...
def week_forecast(city_key: str, include_details: bool = False) -> List[Dict]:
    """
    ...
    day_periods = grid_pick_all_days(periods, num_days=7)
    results = []

    for p in day_periods:
        # Parse the date from the period
        try:
            period_date = dt.datetime.fromisoformat(p.get("startTime", "")).astimezone().date()
            # Use weekday name as label
            label = WEEKDAYS[period_date.weekday()]
        except Exception:
            continue
        ...
        # Get sun times
        sun_times = calculate_sun_times(meta["lat"], meta["lon"], period_date)
...
def find_best_sailing_window(
    city_key: str,
    target_date: Optional[dt.date] = None,
) -> Optional[Dict]:
    """
    Find the best sailing window (time range) for a given day.
    ...
    Returns:
        Dictionary with best window info, or None if no data
    """
    if target_date is None:
        target_date = dt.date.today()

    meta = CITIES[city_key]
    hourly = fetch_hourly_forecast(meta["lat"], meta["lon"])

    if not hourly:
        return None

    # Filter to target date, daytime hours (6am - 8pm)
    day_hours = []
    for p in hourly:
        try:
            period_dt = dt.datetime.fromisoformat(p.get("startTime", "")).astimezone()
            if period_dt.date() != target_date:
                continue
            if not (6 <= period_dt.hour <= 20):
                continue

            wrng = _wind_from_grid(p)
            sky = (p.get("shortForecast") or "").lower()
            temp_f = p.get("temperature")

            rating = compute_rating(wrng, None, sky)

            day_hours.append({
                "hour": period_dt.hour,
                "time": period_dt.strftime("%I%p").lstrip("0").lower(),
                "rating": rating,
                "wind": wrng,
                "sky": sky,
                "temp_f": temp_f,
    ...
    # Find the best contiguous window of 2+ hours
    best_window = None
    best_avg_rating = 0

    for i in range(len(day_hours)):
        for j in range(i + 2, min(i + 6, len(day_hours) + 1)):  # 2-5 hour windows
            window = day_hours[i:j]
            # Check if hours are contiguous
            hours = [h["hour"] for h in window]
            if hours != list(range(hours[0], hours[-1] + 1)):
                continue

            avg_rating = sum(h["rating"] for h in window) / len(window)
    ...
    return {
        ...
        "start_hour": best_window[0]["hour"],
        "end_hour": best_window[-1]["hour"],
        "avg_rating": round(best_avg_rating, 1),
        "hours": best_window,
    }


def _label_to_date(label: str) -> dt.date:
    """Convert a label to a date."""
    today = dt.date.today()
    label_up = label.upper().strip()

    if label_up in ("TODAY", "REST OF TODAY"):
        return today
    if label_up == "TOMORROW":
...
def _pack(
    ...
    prefix: str,
    temp_f: Optional[int] = None,
    wind_raw: Optional[Tuple[int, int]] = None,
    waves_raw: Optional[Tuple[float, float]] = None,
    ...
    Args:
        ...
        wind_raw: Raw wind tuple (lo, hi) in knots
        waves_raw: Raw waves tuple (lo, hi) in feet
        sun_times: Sun times dictionary
        rating_breakdown: Detailed rating breakdown
    ...
    if temp_f is not None:
        ...
        result["temp_c"] = round((temp_f - 32) * 5 / 9)

    # Add raw data for programmatic use
    if wind_raw:
        result["wind_kt"] = {"low": wind_raw[0], "high": wind_raw[1]}
    if waves_raw:
        result["waves_ft"] = {"low": waves_raw[0], "high": waves_raw[1]}

    # Add sun times
    if sun_times:
        result["sun"] = {
            "sunrise": sun_times["sunrise"].strftime("%I:%M%p").lstrip("0").lower() if sun_times.get("sunrise") else None,
            "sunset": sun_times["sunset"].strftime("%I:%M%p").lstrip("0").lower() if sun_times.get("sunset") else None,
            "daylight_hours": sun_times.get("daylight_hours"),
        }

    # Add rating breakdown for verbose mode
    if rating_breakdown:
        result["rating_breakdown"] = rating_breakdown
