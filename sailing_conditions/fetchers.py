...
def http_get(url: str, timeout: int = 20, max_retries: int = 3, backoff: float = 1.0) -> requests.Response:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                print(f"[warn] HTTP request failed (attempt {attempt + 1}/{max_retries}): {e}. Retrying in {wait_time:.1f}s...", file=sys.stderr)
...
def fetch_grid_periods(lat: float, lon: float) -> Optional[List[dict]]:
    ...
    try:
        ...
        if "properties" not in point_data or "forecast" not in point_data["properties"]:
            print(f"[warn] Invalid gridpoint response: missing forecast URL", file=sys.stderr)
            return None
        ...
        if "properties" not in forecast_data or "periods" not in forecast_data["properties"]:
            print(f"[warn] Invalid forecast response: missing periods", file=sys.stderr)
            return None
...
def fetch_ndbc_latest(station: str) -> Optional[dict]:
    ...
    try:
        ...
        # Parse date/time
        YY = 2000 + to_int(data[idx["YY"]])
        MM = to_int(data[idx["MM"]])
        DD = to_int(data[idx["DD"]])
        hh = to_int(data[idx["hh"]])
        mm = to_int(data[idx["mm"]])
        
        if None in (YY, MM, DD, hh, mm):
            print(f"[warn] NDBC date/time parsing failed", file=sys.stderr)
            return None
